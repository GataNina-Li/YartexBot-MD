...
conn.isInit = false
conn.well = false
conn.logger.info(`C A R G A N D O\n`)
...
conn.logger.info(`⚠️ Reinicio necesario, reiniciando...`)
...
if (stopped === 'close' || !conn || !conn.user) return
await purgeSession()
console.log(chalk.cyanBright(`\nAUTOPURGESESSIONS │ ARCHIVOS ELIMINADOS\n`))
}, 1000 * 60 * 60)
setInterval(async () => {
if (stopped === 'close' || !conn || !conn.user) return;
await purgeSessionSB()
console.log(chalk.cyanBright(`\nAUTO_PURGE_SESSIONS_SUB-BOTS │ ARCHIVOS ELIMINADOS\n`));
}, 1000 * 60 * 60)
setInterval(async () => {
if (stopped === 'close' || !conn || !conn.user) return;
await purgeOldFiles()
console.log(chalk.cyanBright(`\nAUTO_PURGE_OLDFILES │ ARCHIVOS ELIMINADOS\n`))
}, 1000 * 60 * 60)
