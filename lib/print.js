...
console.log(`
𝄢 ꒷꒦꒷꒦꒷꒦꒷꒦꒷꒦꒷꒦꒷꒦꒷꒦꒷꒷꒦꒷꒦꒷꒦꒷꒦꒷꒦꒷꒦꒷꒦꒷꒦꒷✧༄
❚ ▸ 𝔹𝕆𝕋 ❱❱ ${chalk.cyan.bold('%s')} 
❚ ▸ ℍ𝕆ℝ𝔸ℝ𝕀𝕆 ❱❱ ${chalk.black.bgGreen('%s')}
❚ ▸ 𝔸ℂℂ𝕀𝕆ℕ ❱❱ ${await formatMessageStubType(m.messageStubType, name_user)}
❚ ▸ 𝕌𝕊𝕌𝔸ℝ𝕀𝕆 ❱❱ ${chalk.white('%s')} / ${chalk.bgMagentaBright.bold(user.role.replace(/\*/g, ''))} / Premium » ${user?.premiumTime > 0 ? '✅' : '❌'}
❚ ▸ ℝ𝔼ℂ𝕌ℝ𝕊𝕆𝕊 ❱❱ ${chalk.yellow('%s%s')}
❚ ▸ %s
❚ ▸ ℙ𝔼𝕊𝕆 ❱❱ ${chalk.red('%s (%s %sB)')}
❚ ▸ 𝕋𝕀ℙ𝕆 ❱❱ ${chalk.bgBlueBright.bold('[%s]')}
𝄢 ꒷꒦꒷꒦꒷꒦꒷꒦꒷꒦꒷꒦꒷꒦꒷꒦꒷꒷꒦꒷꒦꒷꒦꒷꒦꒷꒦꒷꒦꒷꒦꒷꒦꒷✧༄`.trim(),          
...
(m.chat.includes("@g.us") ? `𝔾ℝ𝕌ℙ𝕆 ❱❱ ${chalk.magentaBright(`${m.chat} ~${chat}`)}` : `ℙℝ𝕀𝕍𝔸𝔻𝕆 ❱❱ ${chalk.greenBright(`${m.sender} ~${user.name ? user.name : conn.getName(m.sender)}`)} `),
filesize,
