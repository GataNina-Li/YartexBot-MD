...
let canalNombre = ["𝗔𝗡𝗢𝗧𝗛𝗘𝗥 𝗖𝗛𝗔𝗡𝗡𝗘𝗟 ⟡ INFINITY-WA 💫", "𝗔𝗡𝗢𝗧𝗛𝗘𝗥 𝗖𝗛𝗔𝗡𝗡𝗘𝗟 ⟡ GB - UPDATE 🐈", "𝗔𝗡𝗢𝗧𝗛𝗘𝗥 𝗖𝗛𝗔𝗡𝗡𝗘𝗟 ⟡ Tips sobre GataBot 🤩", "YartexBot-MD ✨"]
...
async function sendList(jid, title, text, footer, buttonText, buffer, listSections, quoted, options) {
    if (buffer) {
        try {
            const type = await conn.getFile(buffer);
            buffer = type.data;
    ...
    for (let i = 0; i < listSections.length; i++) {
        const [sectionTitle, rows] = listSections[i];
        const section = {
            title: sectionTitle || '',
            rows: []
        };
        for (let j = 0; j < rows.length; j++) {
            const [rowTitle, rowId, description] = rows[j];
            const row = {
    ...
    return await conn.sendMessage(jid, listMessage, messageOptions);
}
