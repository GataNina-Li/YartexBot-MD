...
END:VCARD`.trim()
       ...
       // sendButton: {
        ...
        //---
                //-- new
sendButton: {
    async value(jid, text = '', footer = '', buffer, buttons, copyButton, urls, quoted, options) {
        let img, video
	...
	const dynamicButtons = []
        ...
        if (copyButton && Array.isArray(copyButton) && copyButton.length === 2) {
            dynamicButtons.push({
                name: 'cta_copy',
                buttonParamsJson: JSON.stringify({
                    display_text: copyButton[0], 
                    copy_code: copyButton[1] 
                })
            });
        }
